...
add_filter( 'wp_edit_nav_menu_walker', 'cometpro_admin_nav_menu_edit' );
function cometpro_admin_nav_menu_edit () {
    return 'Cometpro_Admin_Nav_Menu_Edit';
}

add_filter( 'wp_setup_nav_menu_item', 'cometpro_get_field_data' );
function cometpro_get_field_data ( $menu_item ) {
    $menu_item->megamenu = get_post_meta( $menu_item->ID, 'menu-item-megamenu', true );
    return $menu_item;
}

add_filter( 'wp_update_nav_menu_item', 'cometpro_set_field_data', 10, 3 );
function cometpro_set_field_data ( $menu_id, $menu_db_id, $args ) {

    if ( is_array( $_REQUEST['menu-item-megamenu'] ) ) {
        $id_value = $_REQUEST['menu-item-megamenu'][ $menu_db_id ];
        update_post_meta( $menu_db_id, 'menu-item-megamenu', $id_value );
    }
}

class Cometpro_Admin_Nav_Menu_Edit extends Walker_Nav_Menu {
            ...
            <div class="menu-item-bar">
                <div class="menu-item-handle">
                    ...
                    <span class="item-controls">
                        ...
                        <sp an class="item-order hide-if-js">
                            <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
                                        array(
                            ...
                            <a href="<?php
                                ...
                                ?>" class="item-move-down" <abbr title="<?php esc_attr_e('Move down'); ?>">&#8595;</abbr></a>						</sp>
